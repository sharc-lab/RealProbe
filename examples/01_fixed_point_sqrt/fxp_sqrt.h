...
void fxp_sqrt(ap_ufixed<W2, IW2>* result, ap_ufixed<W1, IW1>* in_val, int ITER)
{
   ...
   for (int idx = 0; idx <= ITER; idx++)
   {
      ...
      if (SCALE >= 0)
         s = in_val[idx].range(W1-1,0) << (SCALE);
      else
         s = ((in_val[idx].range(W1-1,0) >> (0 - (SCALE + 1))) + 1) >> 1;
      ...
      result[idx].range(W2-1, 0) = ap_uint<W2>(q >> 1);
   }
