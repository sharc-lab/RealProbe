...
static IdentifierLoc ParseXlxPragmaArgument(Parser &P) {
  ...
  // // Do not fail on #pragma HLS inline
  // if (!Tok.isOneOf(tok::identifier, tok::kw_realprobe, tok::kw_register,
  //                  tok::kw_auto, tok::kw_false, tok::kw_true)) {
  //   P.Diag(Tok, diag::warn_unexpected_token_in_pragma_argument)
  //       << PP.getSpelling(Tok) << tok::identifier;
  //   return {SourceLocation(), nullptr};
...
static bool HandleXlxDataflowPragma(XlxPragmaArgParser &PAP, Scope *CurScope,
  ...
  PAP.setPragmaOption(
      ...
      PRAGMA_HLS_DATAFLOW, 
#else 
      {presentId("interval", 1), presentId("disable_start_propagation", 2)}, 
#endif 
      PragmaLoc); 
  if (!PAP.parse())
    return false;
  if (enableXilinxPragmaChecker()) {
    if (PAP.lookup("interval")) {
      ...
      PAP.P.Diag(ii->Loc, diag::warn_extra_pragma_hls_token_ignored) << "interval"
                                                                 << "dataflow";
    }
  }
...
static bool HandleXlxPipelinePragma(XlxPragmaArgParser &PAP, Scope *CurScope,
  ...
  PAP.setPragmaOption("", false,
      ...
      PRAGMA_HLS_PIPELINE, 
#else 
                         {optICEExpr("ii", -1), presentId("rewind", 1),
                          presentId("enable_flush", 2), presentId("off", 3),
  ...
  if (auto Off = PAP.presentedId("off"))
...
static bool HandleInlinePragma(XlxPragmaArgParser &PAP, Scope *CurScope,
  ...
  PAP.setPragmaOption("", false,
      ...
      //TODO, check
                         {presentId("self", 1),presentId("all", 1), 
                          presentId("region", 1),
                          presentId("recursive", 2), presentId("off", 3)},
#endif
                         PragmaLoc);
  if (!PAP.parse())
    return false;
  ...
  bool InlineOff = PAP.presentedId("off");
  ...
  if (!InlineScope) {
    ...
    if (RecursiveInline) {
      ...
      HandleDownwardInlineFunctionPragma(PAP.P, CurScope, PragmaLoc, 1);
      if (InlineOff)
        HandleUpwardInlineFunctionPragma(PAP.P, CurScope, PragmaLoc, true);
      return true;
    }

    // Inline the current function, make sure we are in a function scope
    HandleUpwardInlineFunctionPragma(PAP.P, CurScope, PragmaLoc, InlineOff);
    return true;
  }
  ...
  //   #pragma HLS INLINE region recursive off
  {
    ...
    if (InlineOff)
      HandleDownwardInlineFunctionPragma(PAP.P, CurScope, PragmaLoc, 2);
    else
      HandleDownwardInlineFunctionPragma(PAP.P, CurScope, PragmaLoc, 0);
    return true;
  }
}
...
// 1: recursive
// 2: region off
// static bool HandleRealProbeDownwardInlineFunctionPragma(Parser &P, Scope *CurScope,
//                                                SourceLocation PragmaLoc,
//                                                int IsRecursive) {
//   // auto &PP = P.getPreprocessor();
//   // auto PragmaEndLoc = P.getEndOfPreviousToken();
...
//   auto *DownwardInline = PP.getIdentifierInfo("noinline");
//   auto &Ctx = P.getActions().getASTContext();
//   ArgsUnion Recursive[] = {IntegerLiteral::Create(
//       Ctx, APInt(Ctx.getIntWidth(Ctx.IntTy), IsRecursive), Ctx.IntTy,
//       PragmaLoc)};
...
// static void ApplyNoInlineRecursively(Parser &P, Scope *CurScope, SourceLocation PragmaLoc) {
//     if (!CurScope) return; // Guard against null scopes

//     // Apply 'noinline' to the current function scope
//     auto &PP = P.getPreprocessor();
//     auto *NoInline = PP.getIdentifierInfo("noinline");
//     auto PragmaEndLoc = P.getEndOfPreviousToken();
//     CurScope->getParsedHLSPragmasRef().addNew(NoInline, SourceRange(PragmaLoc, PragmaEndLoc), nullptr,
//                                               PragmaLoc, nullptr, 0, AttributeList::AS_GNU);

//     // Recurse into child scopes (assuming there is a way to get child scopes)
//     for (Scope *ChildScope = CurScope->getFirstChild(); ChildScope; ChildScope = ChildScope->getNextSibling()) {
//         ApplyNoInlineRecursively(P, ChildScope, PragmaLoc);
//     }
// }
...
//                                                int IsRecursive) {
//     if (IsRecursive) {
//         ApplyNoInlineRecursively(P, CurScope, PragmaLoc);
//     } else {
//         // Apply 'noinline' only to the current function scope if not recursive
//         auto &PP = P.getPreprocessor();
//         auto *NoInline = PP.getIdentifierInfo("noinline");
//         auto PragmaEndLoc = P.getEndOfPreviousToken();
//         CurScope->getParsedHLSPragmasRef().addNew(NoInline, SourceRange(PragmaLoc, PragmaEndLoc), nullptr,
...
static bool HandleRealProbeDownwardInlineFunctionPragma(Parser &P, Scope *CurScope,
  ...
  auto *DownwardInline = PP.getIdentifierInfo("xcl_realprobe");
  auto &Ctx = P.getActions().getASTContext();
  ArgsUnion Recursive[] = {IntegerLiteral::Create(
      Ctx, APInt(Ctx.getIntWidth(Ctx.IntTy), IsRecursive), Ctx.IntTy,
      PragmaLoc)};
...
static bool HandleRealProbePragma(XlxPragmaArgParser &PAP, Scope *CurScope,
                               SourceLocation PragmaLoc) {
  // Always set InlineOff to true to disable inlining
  bool InlineOff = true;

  // Parse pragma options normally (this code remains unchanged as you might still want to parse other options)
  PAP.setPragmaOption("", false,
#ifdef TABLEGEN_HLS
      PRAGMA_HLS_REALPROBE,
#else 
      //TODO, check
                         {presentId("self", 1), presentId("all", 1), 
                          presentId("region", 1),
  ...
  // Ignore the specific type scope for inlining, because inlining is turned off globally
  bool typeAll = false;
  bool typeSelf = false;
  bool typeRegion = false;

  // You might still want to maintain this logic to handle any additional behaviors or error handling
  if (enableXilinxPragmaChecker()) {
    if (PAP.presentedId("self") || PAP.presentedId("all") || PAP.presentedId("region")) {
      PAP.P.Diag(PAP.P.getCurToken().getLocation(),
                 diag::warn_deprecated_pragma_option_ignored_by_scout)
          << "Inline"
          << "self/all/region"
  ...
  // Apply no inlining to the current function
  // HandleUpwardInlineFunctionPragma(PAP.P, CurScope, PragmaLoc, InlineOff);

  HandleUpwardInlineFunctionPragma(PAP.P, CurScope, PragmaLoc, true);
  // HandleRealProbeDownwardInlineFunctionPragma(PAP.P, CurScope, PragmaLoc, 2);
...
static bool HandleResetPragma(XlxPragmaArgParser &PAP, Scope *CurScope,
  ...
  PAP.setPragmaOption("", false, 
      ...
      PRAGMA_HLS_RESET,
#else
      {reqVarRefExpr("variable"), presentId("off", 1)},
#endif
                         PragmaLoc);
...
static bool HandleAllocationPragma(XlxPragmaArgParser &PAP, Scope *CurScope,
  ...
  if (ArgsUnion arg = PAP.lookup("function")) { 
    IdentifierLoc *allocType =  arg.get<IdentifierLoc*>(); 
    return HandleFunctionAllocationPragma(allocType,
                                       PAP);
  }
...
static bool HandleInterruptInterfacePragma(Parser &P, Scope *CurScope,
  ...
  for (auto port_ref : subExprs) {
    ArgsUnion args[] = { port_ref, ModeId, PAP.presentedId("register"), PAP.lookup("name"), PAP.lookup("interrupt")};
...
static bool HandleXlxBindOpPragma(XlxPragmaArgParser &PAP, Scope *CurScope,
  ...
  for (Expr *expr : subExprs) {
    ...
    if (isa<DeclRefExpr>(expr) && isa<FunctionDecl>(cast<DeclRefExpr>(expr)->getDecl())) { 
      PAP.P.Diag(PragmaLoc, diag::err_xlx_attribute_invalid_option_and_because)
        << "'variable'" 
        << "bind_op should not apply on return/function"; 
      return false;
    }
    PAP.addDependenceAttribute("xlx_bind_op", args);
  }
...
void Parser::HandleXlxPragma() {
  ...
  auto *Handle =
      StringSwitch<Handler>(str(MaybeName))
          ...
          .CaseLower("realprobe", HandleRealProbePragma)
